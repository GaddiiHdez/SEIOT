...
import { ShieldAlert, ArrowLeft, Trash2, Users, AlertTriangle, KeyRound, Loader2 } from 'lucide-react';
...
const SuperAdminPanel = () => {
    ...
    const handleLimpiarUsuarios = async () => {
        if (!esClaveValida) return;
        if (!confirm('⚠️ ATENCIÓN:\n\nEsta acción eliminará de la base de datos a todos los supervisores y capturistas que no posean rol de Administrador.\n\n¿Deseas continuar?')) return;

        setLoadingUsers(true);
        setResultado(null);
        try {
            const res = await apiFetch('/api/auth/superadmin/limpiar-usuarios', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ confirmacion: confirmInput.trim() })
            });
            ...
            <div className="max-w-4xl mx-auto p-6 md:p-12">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        ...
                        <h1 className="text-2xl font-extrabold tracking-tight mt-1">Consola de Mantenimiento del SuperAdmin</h1>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-8">
                    ...
                    <div className="bg-slate-800/40 backdrop-blur border border-slate-700/50 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row items-center gap-4 justify-between">
                        <div className="flex items-start gap-3 flex-1">
                            ...
                            <div>
                                ...
                                <p className="text-slate-400 text-xs mt-1">Escribe la frase clave <span className="font-mono text-amber-300 font-bold">RESET-DATOS-SEIOT</span> para desbloquear las acciones destructivas del panel.</p>
                            </div>
                        </div>
                        <input
                            type="text"
                            ...
                            placeholder="Ingrese frase de confirmación..."
                    ...
                    {/* Fichas de acciones destructivas */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Tarjeta 1: Vaciar Base de Datos */}
                        ...
                        {/* Tarjeta 2: Limpiar Usuarios */}
                        <div className="bg-slate-800/25 border border-slate-700/40 rounded-2xl p-6 flex flex-col justify-between shadow-lg">
                            <div>
                                <div className="flex items-center gap-2.5 text-amber-500 mb-4">
                                    <Users size={20} />
                                    <h2 className="font-extrabold text-sm uppercase tracking-wider">Limpiar Cuentas Auxiliares</h2>
                                </div>
                                <p className="text-slate-400 text-xs leading-relaxed mb-6">
                                    Elimina de la base de datos a los usuarios que no tengan rol de administrador (capturistas y supervisores).
                                    Útil para limpiar el catálogo de personal de pruebas y asegurar un inicio ordenado de los usuarios definitivos.
                                </p>
                            </div>
                            <button
                                onClick={handleLimpiarUsuarios}
                                disabled={!esClaveValida || loadingUsers}
                                className={`w-full font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 text-xs transition-all active:scale-95 ${
                                    esClaveValida 
                                        ? 'bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-700 hover:to-yellow-700 text-white shadow-md shadow-amber-950/40' 
                                        : 'bg-slate-800 text-slate-500 border border-slate-700/60 cursor-not-allowed'
                                }`}
                            >
                                {loadingUsers ? (
                                    <>
                                        <Loader2 className="animate-spin" size={14} /> ELIMINANDO USUARIOS...
                                    </>
                                ) : (
                                    <>
                                        <Users size={14} /> LIMPIAR CUENTAS AUXILIARES
                                    </>
                                )}
                            </button>
                        </div>
