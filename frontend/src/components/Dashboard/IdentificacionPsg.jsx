...
const IdentificacionPsg = ({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-7 mb-6 transition-all animate-fade-in relative overflow-hidden">
      
      {/* BARRA SUPERIOR DE ACENTO */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-red-700 via-red-800 to-red-900" />

      {/* ENCABEZADO DE SECCIÓN */}
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-150">
        <div className="flex items-center gap-3">
          <span className="w-8 h-8 rounded-xl bg-red-50 text-red-800 font-extrabold flex items-center justify-center text-sm border border-red-100 shadow-sm shrink-0">
            1
          </span>
          <div>
        ...
        {folioActivo && (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 text-slate-700 border border-slate-200 rounded-full text-xs font-bold font-mono">
            <Lock size={12} className="text-slate-500" /> Folio Activo
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div className="relative" ref={containerRef}>
          <label htmlFor="psg-input" className="block text-[11px] font-extrabold text-slate-500 mb-2 uppercase tracking-wider">
            Ingrese Clave PSG
          </label>
          
          <div className={`flex items-center gap-2.5 bg-slate-50/70 p-3 rounded-xl border-2 transition-all duration-200 ${
            folioActivo 
            ...
            <input
              ...
              className={`bg-transparent font-black text-slate-800 w-full outline-none text-sm uppercase tracking-wide placeholder:text-slate-400 placeholder:font-bold ${
                folioActivo ? 'cursor-not-allowed' : ''
              }`}
            />
            {folioActivo && <Lock size={15} className="text-slate-400 shrink-0" />}
          </div>
                ...
                >
                  <div className="space-y-1 pr-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-mono font-black text-red-800 bg-red-100/70 px-2 py-0.5 rounded text-xs tracking-wide group-hover:bg-red-800 group-hover:text-white transition-colors">
                        {s.psg}
                      </span>
                    ...
                    {(s.municipio || s.localidad) && (
                      <p className="text-[11px] text-slate-500 flex items-center gap-1 pl-0.5 font-medium">
                        <Building2 size={12} className="text-slate-400 shrink-0" />
                        {s.municipio || ''} {s.localidad ? `(${s.localidad})` : ''}
                      </p>
                    )}
                  </div>
                  <ChevronRight size={16} className="text-slate-300 group-hover:text-red-700 transition-colors shrink-0" />
                </div>
              ))}
            </div>
          )}
          ...
          {datosPsg && (
            <div className="mt-3 flex items-start gap-3 text-emerald-800 bg-emerald-50/80 p-3.5 rounded-xl border border-emerald-200/80 shadow-sm animate-fade-in">
              <CheckCircle size={18} className="shrink-0 text-emerald-600 mt-0.5" />
              <div className="space-y-0.5">
                <p className="text-xs font-extrabold uppercase tracking-tight text-emerald-950">
                  {datosPsg.nombre_titular}
                </p>
                {datosPsg.municipio && (
        ...
        {(!esVista || folioActivo) && (
          <div>
            <label htmlFor="supervisor-select" className="block text-[11px] font-extrabold text-slate-500 mb-2 uppercase tracking-wider">
              Oficial Asignado
            </label>
            {folioActivo ? (
              <div className="flex items-center gap-3 p-3 bg-slate-100/80 rounded-xl border border-slate-200">
                <div className="w-8 h-8 rounded-lg bg-slate-200 flex items-center justify-center text-slate-600 shrink-0">
                  <UserCheck size={18} />
                </div>
                <div>
                  <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider block">Oficial Responsable</span>
              ...
              <div className="flex items-center gap-2.5 bg-slate-50/70 p-3 rounded-xl border-2 border-slate-200 hover:border-slate-300 focus-within:border-red-700 focus-within:bg-white focus-within:ring-4 focus-within:ring-red-700/10 transition-all duration-200">
                <User size={18} className="text-red-700 shrink-0" />
                <select
                  id="supervisor-select"
                  onChange={handleSupervisorChange}
                  defaultValue=""
                  className="bg-transparent w-full font-bold text-slate-800 text-xs outline-none cursor-pointer"
                >
            ...
            {supervisorSeleccionado && (
              <div className="mt-3 bg-slate-50 p-3.5 rounded-xl border border-slate-200/80 text-xs text-slate-600 space-y-1.5 shadow-sm animate-fade-in">
